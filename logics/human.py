...
def choose_card(cards_qty, on_table):
    suggestion = f'enter card number from 1 to {cards_qty} or press enter to discards/take: ' if on_table > 0 \
...
class Human:
    ...
    def attack(self, table):
        ...
        while self.hand[chosen - 1] not in can_be_added:
            print('This card cannot be added. Choose another one or 0 as a discards action')
            chosen = choose_card(len(self.hand), len(table))
            if chosen == 0:
                return 'discards'
    ...
    def defence(self, table):
        ...
        while not card_bigger(self.hand[chosen - 1], table[-1], self.trump):
            print('You cannot beat by this card. Choose another one or 0 as a take action')
            chosen = choose_card(len(self.hand), len(table))
            if chosen == 0:
                return 'take'
    ...
    def addons(self, table):
        ...
        qty_to_add = self.opp_cards_qty if len(table) // 2 + self.opp_cards_qty < 6 else 6 - (len(table) // 2)
        print('qty', qty_to_add)
        hand = len(self.hand)
        print(hand)
        result = False
        while not result:
            cards_to_add = input(f'Enter cards no. (from 1 to {hand})to  separated by spaces you want to add '
            ...
            if len(cards_to_add) <= qty_to_add:
                cannot_add = [self.hand[i - 1] for i in cards_to_add if self.hand[int(i) - 1] not in can_be_added]
                print('cannot', cannot_add)
                if cannot_add:
                    print(f'These cards cannot be added: {cannot_add}. Try again')
                else:
                    cards_to_add = [self.hand[i - 1] for i in cards_to_add]
                    self.hand = differ(self.hand, cards_to_add)
                    return cards_to_add
            else:
                print(f'Too many cards were chosen. Maximum qty is: {qty_to_add}')
